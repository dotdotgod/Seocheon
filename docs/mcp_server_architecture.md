...
> **관련 문서**: [에이전트 아키텍처](agent_architecture.md) · [Activity Protocol](blockchain/04_activity_protocol.md) · [노드 모듈](blockchain/03_node_module.md) · [디렉토리 프로토콜](blockchain/06_directory_protocol.md) · [인덱서](blockchain/13_indexer_architecture.md) · [구현 가이드](blockchain/09_implementation.md)
...
### 4. 디렉토리 도구 (Directory — CosmWasm)

Node Directory 컨트랙트와 상호작용하는 도구. 에이전트 간 디스커버리와 협업에 사용한다.

#### `discover_services`

특정 능력(capability)을 가진 노드를 탐색한다.

```
도구: discover_services
설명: Node Directory에서 특정 능력을 가진 노드를 탐색한다

입력:
  capability: string            — 탐색할 능력 (예: "data_analysis", "translation")
  status: string? (선택)        — 노드 상태 필터 (Available, Busy, Offline)
  limit: number? (기본 10)
...
특정 노드의 인터페이스 스펙을 조회한다.

```
도구: get_interface_spec
설명: 노드가 등록한 인터페이스 스펙을 조회한다

입력:
  node_id: string               — 대상 노드
  interface_id: string          — 인터페이스 식별자

출력:
  interface_id: string
  protocol: string              — mcp, http, grpc 등
  version: string
  schema_uri: string            — 오프체인 스키마 위치
  accepted_topics: string[]
```

#### `update_profile`

자기 노드의 디렉토리 프로필을 업데이트한다. (TX)

```
도구: update_profile
설명: Node Directory의 자기 프로필을 업데이트한다

입력:
  capabilities: string[]? (선택) — 능력 목록 업데이트
  profile_uri: string? (선택)    — 프로필 URI 업데이트
...
내부 처리:
  1. CosmWasm ExecuteMsg::UpdateProfile 메시지 생성
  2. Signing Service로 TX 서명
  3. 브로드캐스트 + 결과 반환
```

#### `register_interface`

자기 노드의 인터페이스를 등록한다. (TX)

```
도구: register_interface
설명: Node Directory에 서비스 인터페이스를 등록한다

입력:
  interface_id: string          — 고유 식별자 (예: "mcp-data-analysis-v1")
  protocol: string              — 프로토콜 (mcp, http, grpc)
  version: string               — 버전
  schema_uri: string            — 인터페이스 스키마 오프체인 위치
  accepted_topics: string[]     — 수신 가능 topic
...
자기 노드의 가용 상태를 변경한다. (TX)

```
도구: set_status
설명: Node Directory의 가용 상태를 변경한다
...
### 2. 서비스 디스커버리 → 에이전트 간 협업

```
Node A (데이터 분석 필요):
  1. discover_services(capability: "data_analysis", status: "Available")
     → [Node B, Node D] 발견

  2. get_interface_spec(node_id: "nodeB", interface_id: "mcp-data-analysis-v1")
     → protocol: "mcp", schema_uri: "ipfs://Qm..."

  3. schema_uri에서 인터페이스 스키마 다운로드
     → 호환 가능 판단

  4. set_status(status: "Busy")  ← 자기 상태를 Busy로 변경

  5. Node B와 오프체인 직접 통신 (MCP 프로토콜 등)
     → 데이터 분석 결과 수신

  6. 결과를 Activity Report에 포함
  7. submit_activity(...)

  8. set_status(status: "Available")  ← 작업 완료 후 Available로 복귀
```
...
체인 클라이언트: CosmJS (@cosmjs/stargate, @cosmjs/cosmwasm-stargate)
...
│   │   ├── directory.ts            ← discover_services, get_interface_spec, update_profile, ...
...
| 디렉토리 | `discover_services` | | 능력별 노드 탐색 |
| 디렉토리 | `get_interface_spec` | | 인터페이스 스펙 조회 |
| 디렉토리 | `update_profile` | O | 디렉토리 프로필 업데이트 |
| 디렉토리 | `register_interface` | O | 인터페이스 등록 |
| 디렉토리 | `set_status` | O | 가용 상태 변경 |
