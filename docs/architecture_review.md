...
AgentPermissionDecorator(AnteHandler)로 agent_address의 메시지 타입을 화이트리스트 제한. 허용: MsgSubmitActivity, MsgExecuteContract, MsgSend. 그 외 전부 거부. MsgUpdateAgentAddress로 키 교체 + 비활성화 지원 (쿨다운 1 에포크). 상세: [03_node_module.md](blockchain/03_node_module.md) §AgentPermissionDecorator, §MsgUpdateAgentAddress.
