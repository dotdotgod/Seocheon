...
> **담당**: 체인 코어 개발자 / DevOps
> **관련 문서**: [개요](01_overview.md) · [구현 가이드](09_implementation.md) · [재단 전략](../foundation_strategy.md) · [Circuit Breaker](11_circuit_breaker.md) · [전체 목차](README.md)
...
### 유형 D: CosmWasm 컨트랙트 업그레이드

노드 디렉토리 등 CosmWasm 컨트랙트의 업그레이드. 체인 업그레이드와 독립적으로 실행 가능.

```
절차:
  [1] 새 컨트랙트 코드 개발 및 감사
  [2] MsgStoreCode로 새 코드 업로드 (코드 ID 발급)
  [3] MigrateContractProposal 제출 (x/gov)
      ├── contract: 기존 컨트랙트 주소
      ├── code_id: 새 코드 ID
      └── msg: 마이그레이션 메시지 (상태 변환 로직)
  [4] 거버넌스 투표
  [5] 승인 시 컨트랙트 코드 교체 + 마이그레이션 실행

다운타임: 없음 (컨트랙트 마이그레이션은 TX 하나로 완료)
위험도: 중간 (컨트랙트 상태 변환 오류 가능)
롤백: 새 MigrateContractProposal로 이전 코드로 복원
```
