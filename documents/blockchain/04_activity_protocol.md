...
> **구현 상태**: 수수료 수집은 구현 완료. 분배 로직은 x/distribution 확장(동적 이중 보상 풀) 구현 시 함께 적용 예정이다. 현재는 수집된 수수료가 x/activity 모듈 계정에 보관된다.

```
활동 수수료 수집액 (x/distribution 확장 시 적용):
...
에포크 전환 시 (EndBlocker):
  ...
  3. 수집된 수수료 정산 (현재: 모듈 계정 보관. x/distribution 확장 시 80/20 분배 적용)
  4. 다음 에포크에 캐시된 값 적용
```
