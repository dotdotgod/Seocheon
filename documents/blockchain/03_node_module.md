...
│                                                          │
│  제네시스: 0.1 KKOT 할당 (= 1,000,000,000 uppyeo = 10^9)       │
│  → 최대 10억 개 노드 등록 가능 (사실상 무제한)               │
│  → 자금 출처: x/node genesis 별도 mint                       │
│                                                          │
│  등록 시:  풀 → 1 uppyeo → x/staking (자기위임)             │
│  비활성화: x/staking → 1 uppyeo → 풀 (회수, 언본딩 후)     │
│                                                          │
│  풀 잔고 부족 시: MsgRegisterNode 거부                     │
│  풀 보충: x/gov 거버넌스 제안으로 추가 전송                   │
└──────────────────────────────────────────────────────────┘
```
...
│                  (node_feegrant_pool)                     │
│                                                          │
│  제네시스: 1 KKOT 할당 (= 10,000,000,000 uppyeo = 10^10)      │
│  보충: x/gov 거버넌스 제안으로 추가 전송                    │
│                                                          │
│  소모: 가스비 대납 (MsgSubmitActivity)                      │
│  → 노드당 ~0.05 KKOT/에포크, 대부분 1~3개월 내 자립        │
│  → 자립 후 feegrant 미사용 → 풀 소모 감소                  │
└──────────────────────────────────────────────────────────┘
```

### Validator Boost Pool (밸리데이터 부스팅 풀)

초기 밸리데이터 유치를 위한 한시적 인센티브 풀이다. 에포크마다 active validator에게 균등 점진 분배한다.
...
│                (node_boost_pool)                           │
│                                                          │
│  제네시스: 13,500 KKOT (Genesis 27%)                         │
│  목표 기간: 730 에포크 (~2년)                               │
│  에포크당 분배: genesis_balance / target_epochs             │
│  (= 13,500 / 730 ≈ 18.49 KKOT/epoch)                      │
│                                                          │
│  분배: EndBlocker 에포크 전환 시                            │
│  → GetBondedValidatorsByPower()로 active validators 조회  │
│  → 에포크당 분배량을 active validator 수로 균등 분할        │
│  → SendCoinsFromModuleToAccount로 각 validator에 전송     │
│                                                          │
│  종료: 풀 잔액 = 0 → 자동 중단                             │
│  보충: 없음 (한시적 인센티브, 소진 후 인플레이션 보상만)     │
└──────────────────────────────────────────────────────────┘
```

**자연 감소 곡선**: 먼저 참여한 밸리데이터가 더 많은 에포크에 걸쳐 누적 → Tier 시스템 불필요. 밸리데이터가 늘어날수록 인당 분배량 감소. 상세: `blockchain/05_tokenomics.md` §밸리데이터 부스팅 풀
...
TX 수신
├── TX에 MsgRegisterNode만 포함?
│   ├── Yes → MinGasPrices를 0으로 설정하여 가스비 면제
│   │         → 발신자(operator)는 0 토큰으로 TX 제출 가능
│   └── No  → 일반 수수료 처리 (sender 또는 feegrant granter가 지불)
```

Cosmos SDK의 AnteHandler 체인에 데코레이터를 추가하는 표준 패턴이다. SDK 코어를 포크하지 않고 `app.go`에서 설정한다. 등록 TX의 가스비를 면제하여 0원 참여를 가능하게 하며, `max_registrations_per_block`으로 대량 등록을 제한한다. 어뷰징이 문제가 되는 시점에는 거버넌스로 파라미터를 조정할 수 있다.
...
**agent 키 탈취 시 피해 범위**:
- 가짜 활동 제출 (쿼터 10건 이내), agent 지갑 잔고 탈취
- 스테이킹 조작, 거버넌스 투표, 노드 설정 변경, operator 자금 접근은 **불가**
...
| 위험 | 영향 | 완화 방안 |
|------|------|----------|
| Registration Pool 고갈 | 신규 등록 불가 | 거버넌스로 커뮤니티 풀에서 추가 전송; 비활성화 시 1uppyeo 자동 회수 |
...
위임자가 90 에포크(~3개월)마다 자신의 위임을 확인하지 않으면 강제 언본딩하는 메커니즘이다. 90 에포크 주기의 마지막 7 에포크가 **갱신 윈도우**이며, 이 기간에 `MsgConfirmDelegation` TX를 제출해야 한다. 갱신 윈도우 내 확인 시 보상이 끊기지 않고 다음 주기로 연속된다.

### 설계 원칙

- **x/distribution 포크 없음**: 강제 언본딩하면 표준 Cosmos SDK 메커니즘에 의해 보상이 자연 중단된다
- **O(만료 건수) 처리**: 전체 위임을 순회하지 않고, 만료 인덱스만 조회한다
- **자동 초기 설정**: 신규 위임 시 첫 확인 주기가 자동 설정된다 (2번 TX 불필요)
- **갱신 윈도우**: 만료 직전 7 에포크에만 확인 TX를 수락하여, 실질적 능동 참여를 보장한다

### 타임라인

```
                 confirmation_period (90 에포크)
┌─── 정상 기간 (83 에포크) ─────────┐┌── 갱신 윈도우 (7 에포크) ──┐
│                                    ││                            │
0 ···································83····························90
│  위임 유지, 보상 수령               │  위임 유지, 보상 수령        │ 만료
│  MsgConfirmDelegation 거부          │  MsgConfirmDelegation 수락  │ → 강제
│  (갱신 윈도우 밖)                    │  확인 시 → 새 90 에포크 시작 │   언본딩
│                                    │  경고 이벤트 발행            │
└────────────────────────────────────┘└────────────────────────────┘
```

| 구간 | 에포크 범위 (예시) | 위임 상태 | 보상 수령 | MsgConfirmDelegation |
|------|------------------|----------|----------|---------------------|
| 정상 기간 | 0 ~ 83 | 유지 | ✅ | ❌ 거부 (윈도우 밖) |
| 갱신 윈도우 | 83 ~ 90 | 유지 | ✅ | ✅ 수락 → 새 주기 시작, **보상 끊김 없음** |
| 만료 | 90~ | 강제 언본딩 | ❌ (21일 후 토큰 반환) | 불가 (위임 해제됨) |
...
message MsgConfirmDelegation {
  ...
  string delegator_address = 1;  // 위임자 주소
  string validator_address = 2;  // 밸리데이터 주소
}
message MsgConfirmDelegationResponse {
  uint64 expiry_epoch = 1;       // 새 만료 에포크
}
```

**갱신 윈도우 제한**: MsgConfirmDelegation은 **갱신 윈도우 내에서만 수락**된다. 윈도우 밖에서 제출하면 `ErrNotInRenewalWindow` 에러로 거부된다. 이를 통해 위임자가 만료 직전에 능동적으로 자신의 위임을 재확인하는 행위를 강제한다.

```
갱신 윈도우 판정:
  expiry = DelegationConfirmations[(delegator, validator)]
  window_start = expiry - renewal_window
  current_epoch = block_height / epoch_length

  수락 조건: window_start <= current_epoch < expiry
  거부: current_epoch < window_start (아직 갱신 윈도우 아님)
  거부: current_epoch >= expiry (이미 만료됨)
```

확인 TX는 **전용 메시지 타입**만 인정한다. 추가 위임(`MsgDelegate`)이나 재위임(`MsgBeginRedelegate`)은 확인으로 인정하지 않는다. MsgConfirmDelegation은 **TX이므로 가스비가 필요**하며, 이 자체가 능동적 참여의 증명이다.

**확인 시 리셋 규칙**: `new_expiry = current_epoch + confirmation_period`. 갱신 윈도우 시작(에포크 83)에 확인하든, 만료 직전(에포크 89)에 확인하든, 확인 시점부터 새 90 에포크 주기가 시작된다.

### 만료 기반 추적 (Expiry-Based Tracking)

두 개의 컬렉션으로 만료를 추적한다:

```
Storage:
  DelegationConfirmations: Map[(delegator, validator) → expiry_epoch]
    → 특정 위임의 만료 에포크 조회

  PendingExpirations: KeySet[(expiry_epoch, delegator, validator)]
    → 특정 에포크에 만료되는 위임 목록 조회 (에포크 인덱스)

파생 값 (저장하지 않음):
  window_start = expiry_epoch - renewal_window
    → 갱신 윈도우 시작 에포크 (쿼리 시 계산)
```

**동작 흐름**:

```
[1] 신규 위임 (BeforeDelegationCreated hook):
    → expiry = current_epoch + confirmation_period
    → DelegationConfirmations[(delegator, validator)] = expiry
    → PendingExpirations[(expiry, delegator, validator)] = ∅

[2] MsgConfirmDelegation 수신 (갱신 윈도우 내에서만):
    → 위임 존재 확인 (StakingKeeper.GetDelegation)
    → 기존 만료 조회: old_expiry = DelegationConfirmations[(delegator, validator)]
    → 갱신 윈도우 판정: old_expiry - renewal_window <= current_epoch < old_expiry
       └── 윈도우 밖 → ErrNotInRenewalWindow
    → PendingExpirations[(old_expiry, delegator, validator)] 삭제
    → new_expiry = current_epoch + confirmation_period
    → DelegationConfirmations[(delegator, validator)] = new_expiry
    → PendingExpirations[(new_expiry, delegator, validator)] = ∅
    → 이벤트: delegation_confirmed { delegator, validator, expiry_epoch }

[3] EndBlocker (에포크 전환):
    [3-A] 갱신 윈도우 진입 경고:
    → PendingExpirations에서 expiry == current_epoch + renewal_window인 항목 조회
    → 해당 위임들에 대해 경고 이벤트 발행
    → 이벤트: delegation_expiry_approaching { delegator, validator, expiry_epoch }

    [3-B] 강제 언본딩:
    → PendingExpirations에서 expiry <= current_epoch인 항목 순회
    → 각 만료 위임에 대해:
       ├── StakingKeeper.GetDelegation()으로 위임 존재 확인
       │   └── 미존재 → skip (이미 수동 언본딩됨)
       ├── StakingMsgServer.Undelegate() 호출 (전체 shares)
       ├── DelegationConfirmations[(delegator, validator)] 삭제
       └── PendingExpirations[(expiry, delegator, validator)] 삭제
    → 이벤트: delegation_force_unbonded { unbond_count, epoch }

[4] 위임 제거 (BeforeDelegationRemoved hook):
    → old_expiry = DelegationConfirmations[(delegator, validator)]
    → DelegationConfirmations[(delegator, validator)] 삭제
    → PendingExpirations[(old_expiry, delegator, validator)] 삭제
```
...
```
위임자             x/staking            x/node Keeper        PendingExpirations
  │                    │                    │                       │
  │ MsgDelegate        │                    │                       │
  │───────────────────>│                    │                       │
  │                    │ BeforeDelegation   │                       │
  ...
  │                    │                    │──────────────────────>│
  │                    │                    │                       │
  │  (정상 기간: 83 에포크, 보상 수령 중)     │                       │
  │                    │                    │                       │
  │                    │       EndBlocker (에포크 83 = 만료-7)       │
  │                    │                    │ 갱신 윈도우 진입 조회   │
  │                    │                    │──────────────────────>│
  │                    │                    │ → 경고 이벤트 발행     │
  │                    │                    │                       │
  │  (갱신 윈도우: 7 에포크, 보상 계속 수령)  │                       │
  │                    │                    │                       │
  │ MsgConfirmDelegation (윈도우 내 확인!)  │                       │
  │───────────────────────────────────────>│                       │
  │                    │                    │ 윈도우 판정 → OK      │
  │                    │                    │ delete old expiry     │
  │                    │                    │──────────────────────>│
  │                    │                    │ set new expiry (+90)  │
  │                    │                    │──────────────────────>│
  │ Response(new_expiry) ← 보상 끊김 없이 다음 주기 시작!            │
  │<───────────────────────────────────────│                       │
  │                    │                    │                       │
  ════════════════════════════════════════════════════════════════
  │  (갱신 윈도우에서도 미확인 시)           │                       │
  │                    │                    │                       │
  │                    │       EndBlocker (에포크 90 = 만료)         │
  │                    │                    │ query expired         │
  │                    │                    │──────────────────────>│
  │                    │                    │ [(delegator,validator)]│
  │                    │                    │<──────────────────────│
...
`x/node`는 기존 `x/staking` hook 인터페이스를 구현하고 있으며, delegation 관련 hook은 현재 no-op이다. 능동 위임은 이 hook을 활용한다:

| Hook | 현재 | 능동 위임 구현 |
|------|------|--------------|
| `BeforeDelegationCreated` | no-op | 자동 설정 (expiry = current + period) |
| `BeforeDelegationRemoved` | no-op | 확인 기록 정리 (두 컬렉션에서 삭제) |
| `AfterValidatorBonded` | ACTIVE 전환 | 변경 없음 |
| `AfterValidatorBeginUnbonding` | REGISTERED 복귀 | 변경 없음 |

### 파라미터

| 파라미터 | 타입 | 기본값 | 거버넌스 | 설명 |
|---------|------|--------|---------|------|
| `delegation_confirmation_period` | uint64 | 90 | ✅ | 전체 확인 주기 (에포크 수). 0 불가 |
| `delegation_renewal_window` | uint64 | 7 | ✅ | 갱신 윈도우 크기 (에포크 수). 주기 마지막 N 에포크. 0 불가, period 미만이어야 함 |

**갱신 윈도우 계산**: `window_start = expiry - renewal_window`. 기본값 기준 에포크 83~90이 갱신 윈도우.

**파라미터 변경 시**: 기존 위임의 만료 에포크는 변경되지 않는다. 새 확인(`MsgConfirmDelegation`)부터 변경된 주기가 적용된다.

### 이벤트

| 이벤트 타입 | 속성 | 발생 시점 |
|------------|------|----------|
| `delegation_confirmed` | `delegator`, `validator`, `expiry_epoch` | MsgConfirmDelegation 처리 시 |
| `delegation_expiry_approaching` | `delegator`, `validator`, `expiry_epoch` | EndBlocker에서 갱신 윈도우 진입 시 (경고) |
| `delegation_force_unbonded` | `unbond_count`, `epoch` | EndBlocker에서 만료 위임 강제 언본딩 시 |

### 쿼리

```protobuf
// 특정 위임의 확인 상태 조회
rpc DelegationConfirmation(QueryDelegationConfirmationRequest)
    returns (QueryDelegationConfirmationResponse);

message QueryDelegationConfirmationRequest {
  string delegator_address = 1;
  string validator_address = 2;
}
message QueryDelegationConfirmationResponse {
  uint64 expiry_epoch = 1;           // 만료 에포크
  uint64 current_epoch = 2;          // 현재 에포크
  bool   confirmed = 3;              // expiry > current (유효 여부)
  bool   in_renewal_window = 4;      // 갱신 윈도우 중인지 (MsgConfirmDelegation 수락 가능)
  uint64 renewal_window_start = 5;   // 갱신 윈도우 시작 에포크
}
```

**인덱서/대시보드 활용**: `in_renewal_window = true`인 위임에 대해 알림을 발송하여 위임자가 MsgConfirmDelegation을 제출하도록 유도한다. 갱신 윈도우 중 확인하면 보상이 끊기지 않으므로, 성실한 위임자는 불이익 없이 연속 보상을 받는다.

### 가스비

MsgConfirmDelegation은 표준 Cosmos 트랜잭션이다:
- **가스비 필요**: 위임자 계정에 가스비 상당의 토큰이 있어야 한다
- **가스 비용**: 단순 상태 업데이트 (2건의 컬렉션 삭제 + 2건의 컬렉션 쓰기). 일반 MsgSend와 유사한 수준
- **의도적 비용**: 가스비 자체가 능동적 참여의 최소 증명. 무비용이면 자동화 봇으로 무의미해짐

### 엣지 케이스

| 케이스 | 처리 |
|--------|------|
| 갱신 윈도우 전에 확인 시도 | `ErrNotInRenewalWindow` 거부. 윈도우 시작까지 대기 필요 |
| 갱신 윈도우 내 확인 | 새 expiry = current_epoch + period. **보상 끊김 없이 연속** |
| 갱신 윈도우에도 미확인 | expiry 도달 시 강제 언본딩 (21일 unbonding) |
| 만료 전에 수동 언본딩 | `BeforeDelegationRemoved` hook이 확인 기록 정리 → EndBlocker에서 skip |
| 만료 시점에 이미 언본딩 중 | `GetDelegation()` 실패 → skip (이중 언본딩 방지) |
| 에포크 중간에 신규 위임 | `BeforeDelegationCreated` hook이 자동 설정 (expiry = current + period) |
| 같은 validator에 추가 위임 | 기존 확인 기록 유지 (추가 위임은 기존 delegation에 merge됨) |
| 파라미터 변경 (period 90→60) | 기존 만료 에포크 불변. 다음 확인부터 신규 주기 적용 |
| 파라미터 변경 (window 7→14) | 기존 만료 에포크 불변. 다음 확인부터 신규 윈도우 적용 |
| 강제 언본딩 실패 (잔액 부족 등) | 로그 기록 + 다음 에포크 재시도. 두 컬렉션에서 삭제하지 않음 |

### Genesis

```protobuf
message DelegationConfirmation {
  string delegator_address = 1;
  string validator_address = 2;
  uint64 expiry_epoch = 3;       // 만료 에포크
}

// GenesisState에 추가:
repeated DelegationConfirmation delegation_confirmations = 7;
```

`InitGenesis`: `DelegationConfirmations` + `PendingExpirations` 양쪽에 복원.
`ExportGenesis`: `DelegationConfirmations` 순회하여 모든 확인 기록 export.
