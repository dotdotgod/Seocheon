...
> **관련 문서**: [핵심 개념](02_core_concepts.md) · [Activity Protocol](04_activity_protocol.md) · [토큰 이코노믹스](07_tokenomics.md) · [체인 업그레이드](10_chain_upgrade.md) · [Circuit Breaker](11_circuit_breaker.md) · [전체 목차](README.md)
...
Cosmos SDK v0.50의 `x/staking` 모듈에는 다음 제약이 존재한다:
...
│                                                          │
│  제네시스: 1,000 KKOT 할당 (= 1,000,000,000 usum)          │
...
신규 노드의 가스비를 대납하는 재원이다. 블록 보상에서 `feegrant_tax` (기본 0.5%)를 자동 적립하여 고갈 없이 운영된다.
...
│                                                          │
│  제네시스: 10,000 KKOT 할당                                │
│  자동 충전: 매 에포크 블록 보상의 0.5% 적립 (feegrant_tax) │
│  → 인플레이션 7% 기준 연간 ~175,000 KKOT 유입              │
│  → 연간 ~19,000개 신규 노드 지원 가능                      │
│                                                          │
│  소모: 가스비 대납 (MsgSubmitActivity, MsgExecuteContract) │
│  → 노드당 ~0.05 KKOT/에포크, 대부분 1~3개월 내 자립        │
│  → 자립 후 feegrant 미사용 → 풀 소모 감소                  │
│                                                          │
│  feegrant_tax 변경: 거버넌스 파라미터                      │
└──────────────────────────────────────────────────────────┘
```
...
```
RegistrationFeeDecorator (AnteHandler 체인에 삽입):

TX 수신
├── TX에 MsgRegisterNode 또는 MsgRenewFeegrant만 포함?
│   ├── Yes → 가스비를 node_feegrant_pool 모듈 계정에서 차감
│   │         → 발신자(operator)는 0 토큰으로 TX 제출 가능
│   └── No  → 일반 수수료 처리 (sender 또는 feegrant granter가 지불)
```

Cosmos SDK의 AnteHandler 체인에 데코레이터를 추가하는 표준 패턴이다. SDK 코어를 포크하지 않고 `app.go`에서 설정한다.
...
│           period:            17,280 블록 (1 에포크),
│           period_spend_limit: 1,000,000 usum (1 KKOT),
│           expiration:        current_block + 3,110,400 (6개월)
│         },
│         allowed_msg_types: [
│           MsgSubmitActivity,
│           MsgExecuteContract
...
│
└── [6] 응답 반환
    └── MsgRegisterNodeResponse { node_id, validator_address, feegrant_expiry }
```
...
Operator: agent_share 30% → 15%로 변경 요청 (MsgUpdateNodeAgentShare)
  │
  ├── max_agent_share_change_rate = 5%p/에포크 (1 에포크 = 1일)
  │   → 15%p 변경은 3 에포크(3일)에 걸쳐 진행
  │
  ├── 에포크 1: 30% → 25% (pending, 1일 유예)
  │   └── 대시보드에 "변경 예정" 표시
  │   └── 위임자가 확인, 동의하지 않으면 언딜리게이션
  │
  ├── 에포크 2: 25% → 20% (pending, 1일 유예)
  │
  └── 에포크 3: 20% → 15% (pending, 1일 유예) → 최종 적용
```

위임자의 보호 수단은 **"발로 투표"(위임 철회)**이다. 체인이 변경을 강제로 막지 않지만, 충분한 공지 기간과 점진적 변경으로 위임자의 대응 시간을 보장한다.
...
노드 운영자의 등록 절차:
  ...
  4. MsgRegisterNode TX 제출 (feegrant로 가스비 0)
...
| Feegrant Pool 고갈 | 신규 노드 가스비 대납 불가 | feegrant_tax로 블록 보상에서 자동 충전 (연간 ~175K KKOT) |
...
```
0원 참여 플로우:
  1. 노드 등록 (MsgRegisterNode, 가스비: feegrant_pool에서 차감)
...
```
MsgRenewFeegrant 수신 (operator 서명, 가스비: feegrant_pool에서 차감)
│
...
│
├── [4] 갱신 실행
│   └── feegrantKeeper.GrantAllowance() — 동일 파라미터로 6개월 연장
│
└── [5] 응답 반환
    └── MsgRenewFeegrantResponse { new_expiry }
