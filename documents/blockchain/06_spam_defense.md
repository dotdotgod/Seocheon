...
> **관련 문서**: [핵심 개념](02_core_concepts.md) · [노드 모듈](03_node_module.md) · [Activity Protocol](04_activity_protocol.md) · [토큰 이코노믹스](05_tokenomics.md) · [구현 가이드](07_implementation.md) · [Circuit Breaker](09_circuit_breaker.md) · [전체 목차](README.md)
...
자비 부담 노드:
  self_funded_quota = 100                     ← 거버넌스 파라미터

feegrant 노드:
  feegrant_quota = 10                         ← 거버넌스 파라미터 (포화 시 effective_quota로 축소)
    + period_spend_limit = 10,000,000,000 uppyeo (1 KKOT/에포크) ← x/feegrant 금액 제한 (이중 제한)
...
**feegrant 노드 판별**: `x/activity` keeper가 제출자의 agent_address에 대해 Feegrant Pool(granter) → submitter(grantee) 경로로 `x/feegrant`의 `Allowance` 쿼리를 실행하여 feegrant 수혜 여부를 확인한다.
...
선택적 등록 보증금:
  registration_deposit = 0 uppyeo             ← 초기값 0 (거버넌스로 활성화 가능)
...
└── allowance: PeriodicAllowance
    ├── period: 24시간 (~1 에포크)
    ├── period_spend_limit: 10,000,000,000 uppyeo (1 KKOT/에포크)
...
| **노드 등록** | | | |
| `min_self_delegation` | x/staking | 1 uppyeo | CreateValidator 시 자동 설정 |
| `max_registrations_per_block` | x/node | 5 | 거버넌스 |
| `registration_cooldown_blocks` | x/node | 100 | 거버넌스 |
| `registration_deposit` | x/node | 0 uppyeo | 거버넌스 |
| **Agent 권한** | | | |
...
| `base_activity_fee` | x/activity | 10,000,000,000 uppyeo (1 KKOT) | 거버넌스 |
| `fee_exponent` | x/activity | 5000 (=0.5) | 거버넌스 |
| `max_activity_fee` | x/activity | 1,000,000,000,000 uppyeo (100 KKOT) | 거버넌스 |
